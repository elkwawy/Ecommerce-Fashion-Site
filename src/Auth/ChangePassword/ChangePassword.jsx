...
export default function ResetPassword() {
    ...
    const validationSchema = yup.object({
        ...
        newPassword: yup.string().required("password is required").matches(/^(?=.*[A-Z]).{8,}$/,'Min 8 chars, with capital letter'),
    })
      ...
      const {data} = await axios.request(options)
       showToast("success","password changed successfully")
        navigate("/")
         ...
         showToast("error",error.response.data?.message || "problem with change password")
      }
    }
  ...
  return <>
   <section className='min-h-screen text-center '>
    <Formik
             ...
             />      
                            <ButtonForm type='submit' className='w-full'>change password</ButtonForm>
                   </Form>
                               )}
                                    </Formik>
                </section>
