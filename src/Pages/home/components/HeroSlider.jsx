...
const HeroSlider = () => {
  ...
  useEffect(() => {
    const images = [
      "/heroSlider/men.webp",
      "/heroSlider/women.webp",
      "/heroSlider/children.webp",
    ];
  ...
  return (
    <div className="relative w-full   overflow-hidden">
      ...
      <div className=" absolute flex flex-col justify-center items-center gap-8 top-0 left-0 bg-black/[33%] w-full h-full  z-20">
        <h1 className="font-bold text-white text-[50px] min-[550px]:text-[80px] leading-tight text-center ">
          Youthful <br /> Summer style
        </h1>
        <p className="text-white text-center text-[20px] min-[550px]:text-[24px]">
        ...
        {[1, 2, 3].map((_, index) => (
          <div
            ...
            className={` rounded-full transition-all duration-300  ${
              currentSlide === index
                ? "bg-black w-[34px] h-[5px] rounded-full mt-[2px]"
                : "bg-[#D9D9D9] h-[8px] w-[8px]"
            } cursor-pointer`}
            onClick={() => {
              changeSlide(index);
              setCurrentSlide(index);
