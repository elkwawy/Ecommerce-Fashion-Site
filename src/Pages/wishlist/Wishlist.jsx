        ...
        <section className='pb-8'>
          <div className='w-[90%] m-auto'>
            <div className='border-b-2 pb-4 pt-4 flex justify-between items-center'>
              ...
              <button className='text-sm text-nowrap border-2 rounded px-4 py-2 md:px-6'>
                Sort by <span className='font-bold'>Newest <IoIosArrowDown className='inline' /></span>
              </button>
            </div>
                    ...
                    <div className='flex gap-2 justify-start'>
                      {item.colors?.length > 0 ? (
                        item.colors.map((color, index) => (
                          <div className='w-8 h-8 rounded-full border-2' style={{ backgroundColor: color }} key={index}></div>
                        ))
                      ) : (
        ...
        <section className=''>
          <div className='w-[90%] m-auto'>
            ...
            <div className='flex flex-col justify-center items-center text-center  min-h-screen'>
              ...
              <p className='text-sm mb-4 font-bold'>Personalize your shopping experience with your wishlist</p>
              <Link to="/" className='w-[21.125rem] bg-black text-white py-2 px-6 rounded'>CONTINUE SHOPPING</Link>
            </div>
          </div>
        </section>
      )}
